...
def compute_displacement_vectors(ref_img: np.ndarray,
    ...
    for i, y in enumerate(y_coords):
        ...
        for j, x in enumerate(x_coords): 
            ...
            if corr_matrix is None:
                ...
                from utils.dic_util import gaussian_subpixel_refinement
    ...
    #filter
    from utils.dic_util import filter 
...
def veg_segmentation(X: np.ndarray,
    ...
    if veg_mask is not None:
        ...
        # Set displacement vectors to zero where vegetation is detected
        U[mask_indices] = None
        V[mask_indices] = None
    return U, V
...
def save_displacement_to_csv(X, Y, U, V, filename):
    ...
    with open(filename, mode='w', newline='') as f:
        ...
        for i in range(X.shape[0]):
            writer.writerow([X[i], Y[i], U[i], V[i]])
    return None
...
def main(ref_path:str,
    ...
    if ref_path or def_path is None:
        raise ValueError(f"ref_path or def_path cannot be empty")
